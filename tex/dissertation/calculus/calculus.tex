    ...
    \subsubsection{Overview and Analysis}\label{sssec:calculus-analysis}
        ...
        \begin{breakablealgorithm}
            ...
            \begin{algorithmic}[1]
                ...
                \Function{Reduce}{$P$}
                    \ForEach{$i \in inputs(P)$} \Comment{$inputs(P) \equiv \{x \in solos(P) \;|\; x \text{ an input}\}$}
                        \ForEach{$o \in outputs(P)$} \Comment{$outputs(P) \equiv \{x \in solos(P) \;|\; x \text{ an output}\}$}
        ...
        \begin{breakablealgorithm}
            ...
            \begin{algorithmic}[1]
                ...
                \Function{Reduce}{$P$}
                    ...
                    \ForEach{$i \in \bigcup_{r \in reps(P)} inputs(r)$} \Comment{$reps(P) \equiv \{x \in P \;|\; x \text{ a replicator}\}$}
                        \ForEach{$o \in \bigcup_{r \in reps(P)} outputs(r)$}
                            \If{$i$ agrees with $o$}
                                \State$\sigma \defeq$ \Call{Fuse}{$i$, $o$, $bn(P) \cup bn(r_i) \cup bn(r_o)$} \Comment{$r_x \equiv r \in reps(P) \;|\; x \in solos(r)$}
                                \If{$\sigma \neq \textit{none}$}
